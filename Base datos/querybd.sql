...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 06-08-2020 a las 23:29:13
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AGREGAR_PUB`(IN `nombreProducto` VARCHAR(200), IN `caracteristicas` VARCHAR(500), IN `idCategoria` INT, IN `ptipo` VARCHAR(50), IN `pprecio` INT, IN `pidPersona` INT, IN `pidMoneda` INT, IN `idAnuncio` INT, IN `url` VARCHAR(500), IN `accion` VARCHAR(100), OUT `mensaje` VARCHAR(100))
SP:BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CAMBIAR_TIEMPO_USUARIO_ADMINISTRADOR`(IN `cantidadDiasIN` INT, OUT `mensaje` VARCHAR(100), OUT `codigo` VARCHAR(2), OUT `cantidadDiasOut` INT)
SP:BEGIN
    DECLARE conteo INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CAMBIAR_TIEMPO_USUARIO_NORMAL`(IN `cantidadDiasIN` INT, OUT `mensaje` VARCHAR(100), OUT `codigo` VARCHAR(2), OUT `cantidadDiasOut` INT)
SP:BEGIN
    DECLARE conteo INT;
    DECLARE idTipoUsuarioVar INT;
    DECLARE cantidadDias INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CANTIDAD_ADMIN`(IN `accion` VARCHAR(45), OUT `mensaje` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE id INT;
  DECLARE tempMensaje VARCHAR(100);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CATEGORIAS`(IN `nombreCategoria` VARCHAR(50), IN `idcategoria` INT, IN `accion` VARCHAR(45), IN `pestado` VARCHAR(2), OUT `mensaje` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE id INT;
  DECLARE tempMensaje VARCHAR(100);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DENUNCIAS`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DETALLE_PUBLICACION`(IN `idUsuarioDaLike` INT, IN `pidCalificacion` INT, IN `idPublicacion` INT, IN `idUsuarioRecibeLike` INT, IN `cantidad` INT, IN `prazones` VARCHAR(200), IN `accion` VARCHAR(45), IN `pestado` VARCHAR(2), OUT `mensaje` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE conteo2 INT;
  DECLARE id INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_FAVORITOS`(IN `IN_idPersona` INT, IN `IN_idAnuncio` INT, IN `accion` VARCHAR(20), OUT `codigo` INT, OUT `mensaje` VARCHAR(100))
SP:BEGIN
    DECLARE conteo INT;
    DECLARE id INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LOGIN`(IN `pcorreo` VARCHAR(50), IN `pcontrasenia` VARCHAR(50), OUT `pid` INT, OUT `mensaje` VARCHAR(100), OUT `existe` INT, OUT `contrasenaCorrecta` INT, OUT `estadoRegistro` INT, OUT `esUsuarioAdmin` INT)
SP:BEGIN
  DECLARE conteo INT;
  DECLARE contra INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_PERFIL_ADMIN`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_PRODUCTOS_Y_SERVICIOS`(IN `nombrePS` VARCHAR(50), IN `descripcion` VARCHAR(1000), IN `ptipo` VARCHAR(10), IN `pidcategoria` INT, IN `pidProducto` INT, IN `accion` VARCHAR(45), IN `pestado` VARCHAR(2), OUT `mensaje` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE id INT;
  DECLARE tempMensaje VARCHAR(100);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REGISTRO_USUARIO`(
                    IN ppNombre VARCHAR(50),
                    IN psNombre VARCHAR(50),
                    IN ppApellido VARCHAR(50),
                    IN psApellido VARCHAR(50),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REGISTRO_USUARIO_ADMINISTRADOR`(
                    IN ppNombre VARCHAR(50),
                    IN psNombre VARCHAR(50),
                    IN ppApellido VARCHAR(50),
                    IN psApellido VARCHAR(50),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REPORTES`(IN `accion` VARCHAR(45), OUT `mensaje` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE tempMensaje VARCHAR(100);
  SET autocommit=0;  
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UBICACION`(IN `nombredepto` VARCHAR(45), IN `idDepto` VARCHAR(45), IN `accion` VARCHAR(30), OUT `codeMessage` INT, OUT `message` VARCHAR(100))
SP:BEGIN
  DECLARE conteo INT;
  DECLARE tempMensaje VARCHAR(100);
  SET autocommit=0;  
